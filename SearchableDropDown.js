...
export default class SearchableDropDown extends Component {
  ...
  searchedItems = searchedText => {
    ...
    if (!setSort && typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        return (
          item.label.toLowerCase().indexOf(searchedText.toLowerCase()) > -1
        );
      };
    }
  ...
  renderTextInput = () => {
    ...
    const oldSupport = [
      ...
      {
        key: 'value',
        val: this.state.item ? this.state.item.label : '',
      },
      {
        key: 'style',
    ...
    return (
      <TextInput
        ...
        onBlur={e => {
          ...
          console.log(this.state.item, 'TextInput');
          this.setState({focus: false, item: this.state.item});
        }}
      />
    );
  };
